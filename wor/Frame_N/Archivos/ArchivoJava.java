...
public class ArchivoJava {
	...
	protected class tipoArg {
		...
		public String toString(){return this.tipo.getSimpleName() + " " + this.nomre;}
	}
	...
	private String getVariables(){
		...
		if(this.variables != null)
			for(tipoVariable aux: this.variables){
				...
				ret += " " + aux.tipo.getSimpleName() + " " + aux.nombre;
				if (aux.tipo == String.class)
					ret += " = \"" + aux.valor + "\";\n";
				else ret += " = " + aux.valor + ";\n";
			}
	...
	private String getMetodo(tipoMetodo proc){
		...
		if (proc.tipo != null) ret += " " + proc.tipo.getSimpleName();
		ret += " " + proc.nombre + "(";
		...
		try {
			...
			if (this.paqueteDIR != null)
				if (this.paqueteDIR != "")
					...
					javaFile.escribir("package " + "tap.generated.server" + ";\n\n");
					javaFile.escribir(this.getComentarioAgregado());
					javaFile.escribir(getIncludes());
					
					javaFile.escribir("@Path(\"/service\")"+"\n");
